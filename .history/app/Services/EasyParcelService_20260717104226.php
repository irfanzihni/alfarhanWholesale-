...
class EasyParcelService
{
    ...
    public function createShipment($order)
    {
        ...
        // If API key is not configured, we can only mock.
        // (sandbox is for demo; when forcing real EasyParcel, set EASYPARCEL_SANDBOX=false)
        if (empty($this->apiKey)) {
            Log::info('EasyParcel API key not configured. Generating mock shipment booking.');
            return [
                'status' => 'Success',
        ...
        if ($this->sandbox) {
            Log::info('EasyParcel sandbox active. Generating mock shipment booking.');
            return [
                'status' => 'Success',
                'easyparcel_order_id' => 'EP-MOCK-' . strtoupper(uniqid()),
                'tracking_code' => 'MY-MOCK-' . $order->id . '-' . rand(100000, 999999),
